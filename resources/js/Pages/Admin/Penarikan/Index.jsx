...
const STATUS_STYLE = {
    pending: 'bg-amber-50 text-amber-700 border border-amber-200 dark:bg-amber-950/40 dark:text-amber-400 dark:border-amber-800/50',
    ...
    ditolak: 'bg-red-50 text-red-700 border border-red-200 dark:bg-red-950/40 dark:text-red-400 dark:border-red-800/50',
};
...
export default function PenarikanIndex({ penarikan, filter }) {
    ...
    return (
        <AdminLayout title="Penarikan Saldo">
            ...
            {flash?.success && (
                <div className="mb-5 flex items-center gap-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-emerald-700 dark:border-emerald-800/50 dark:bg-emerald-950/40 dark:text-emerald-400">
                    ...
                    <table className="w-full text-left text-xs">
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                            {penarikan.data.length === 0 && (
                                <tr>
                                    <td colSpan={6} className="select-none px-4 py-16 text-center text-gray-400 dark:text-gray-500">
                                        <div className="flex flex-col items-center justify-center">
                                            <Wallet size={32} className="mb-2 text-gray-300 dark:text-gray-600" />
                            ...
                            {penarikan.data.map((item) => (
                                <tr key={item.id} className="transition-colors hover:bg-gray-50/60 dark:hover:bg-slate-800/50">
                                    <td className="px-5 py-3.5">
                                        <p className="font-semibold text-gray-900 dark:text-gray-100">{item.user?.name ?? '-'}</p>
                                        <p className="text-[11px] text-gray-500 dark:text-gray-400">{item.user?.email ?? '-'}</p>
                                    </td>
                                    <td className="px-5 py-3.5">
                                        <span className="text-sm font-bold text-green-700 dark:text-emerald-400">
                                            {formatRupiah(item.jumlah)}
                                        </span>
                                    </td>
                                    <td className="px-5 py-3.5 text-gray-600 dark:text-gray-300">
                                        <div className="flex items-center gap-1.5 font-medium text-gray-800 dark:text-gray-200">
                                            <Building2 size={13} className="shrink-0 text-green-700 dark:text-emerald-400" />
                                            <span>{item.nama_bank}</span>
                                            <span className="text-gray-300 dark:text-gray-600">&middot;</span>
                                            <span className="font-mono text-gray-600 dark:text-gray-400">{item.nomor_rekening}</span>
                                        </div>
                                    ...
                                    <td className="select-none px-5 py-3.5">
                                        <span
                                            className={`inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-semibold ${
                                                STATUS_STYLE[item.status] ?? 'border border-gray-200 bg-gray-100 text-gray-600 dark:border-gray-800 dark:bg-slate-800 dark:text-gray-400'
                                            }`}
                                        >
                                            {STATUS_LABEL[item.status] ?? item.status}
                                        </span>
                                        {item.status === 'ditolak' && item.catatan && (
                                            <p className="mt-1.5 rounded-md border border-red-100 bg-red-50 p-1.5 text-[11px] italic text-red-600 dark:border-red-900/50 dark:bg-red-950/30 dark:text-red-400">
                                                "{item.catatan}"
                                            </p>
                                        )}
                                    </td>
                                        ...
                                        {item.status === 'pending' ? (
                                            <div className="flex items-center justify-end gap-1.5">
                                                <button
                                                    ...
                                                    className="inline-flex items-center gap-1 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-xs font-bold text-emerald-700 transition hover:bg-emerald-600 hover:text-white dark:border-emerald-800/60 dark:bg-emerald-950/50 dark:text-emerald-400 dark:hover:bg-emerald-600 dark:hover:text-white"
                                                >
                                                    <Check size={14} />
                                                    Setujui
                                                </button>
            ...
            {penarikan.data.length > 0 && (
                <div className="mt-5 flex flex-wrap items-center justify-between gap-3 select-none">
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                        Menampilkan <span className="font-semibold text-gray-700 dark:text-gray-200">{penarikan.from}</span> sampai{' '}
                        <span className="font-semibold text-gray-700 dark:text-gray-200">{penarikan.to}</span> dari{' '}
                        <span className="font-semibold text-gray-700 dark:text-gray-200">{penarikan.total}</span> data
                    </p>
            ...
            {approveTarget && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 backdrop-blur-sm">
                    <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl dark:border dark:border-gray-800 dark:bg-slate-900">
                        ...
                        <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">Setujui Penarikan?</h3>
                        <p className="mt-1.5 text-sm leading-relaxed text-gray-500 dark:text-gray-400">
                            Saldo mitra <span className="font-semibold text-gray-800 dark:text-gray-200">{approveTarget.user?.name}</span> akan
                            dipotong sebesar <span className="font-semibold text-emerald-600 dark:text-emerald-400">{formatRupiah(approveTarget.jumlah)}</span> dan
                            dana ditransfer ke rekening <span className="font-semibold text-gray-800 dark:text-gray-200">{approveTarget.nama_bank}</span>.
                        </p>
            ...
            {rejectTarget && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 backdrop-blur-sm">
                    <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl dark:border dark:border-gray-800 dark:bg-slate-900">
                        ...
                        <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">Tolak Penarikan</h3>
                        <p className="mt-1.5 text-sm leading-relaxed text-gray-500 dark:text-gray-400">
                            Permintaan penarikan <span className="font-semibold text-red-600 dark:text-red-400">{formatRupiah(rejectTarget.jumlah)}</span> dari{' '}
                            <span className="font-semibold text-gray-800 dark:text-gray-200">{rejectTarget.user?.name}</span> akan ditolak.
                        </p>
                        ...
                        <div className="mt-4">
                            ...
                            <textarea
                                ...
                                className="w-full rounded-xl border border-gray-200 px-3.5 py-2.5 text-sm text-gray-800 focus:border-red-500 focus:outline-none focus:ring-1 focus:ring-red-500 dark:border-gray-800 dark:bg-slate-800 dark:text-gray-200 dark:placeholder-gray-500"
                            />
                        </div>
