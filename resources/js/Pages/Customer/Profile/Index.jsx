...
export default function ProfileIndex({ user }) {
    ...
    const [darkMode, setDarkMode] = useState(
        () =>
            ...
            localStorage.getItem('titipsini_theme') === 'dark'
    );
    ...
    // --- Sub-Components (Inline for cleaner structure) ---
    ...
    const MenuLink = ({ href, icon: Icon, label, last }) => (
        <Link
            href={href}
        ...
        >
            <span className="flex items-center gap-3 text-sm font-medium text-gray-700 dark:text-gray-200">
                <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-green-50 text-[#15803d] dark:bg-green-950/40 dark:text-[#22c55e] transition group-hover:scale-110">
                    <Icon size={17} />
                </div>
                {label}
            </span>
            <ChevronRight
                size={16}
                className="text-gray-300 dark:text-gray-600 transition group-hover:translate-x-0.5"
            />
        </Link>
    );
    ...
    return (
        <CustomerLayout title="Profil" backHref="/app/dashboard">
            ...
            <div className="pb-8">
                {/* Header Card */}
                <header className="relative overflow-hidden bg-gradient-to-br from-[#16a34a] via-[#15803d] to-[#14532d] dark:from-green-800 dark:via-green-700 dark:to-green-950 px-6 pt-6 pb-12 text-white rounded-b-[36px] shadow-md">
                ...
                <main className="px-4 -mt-6 relative z-20">
                    {/* Personal Data Card */}
                    <section className="relative rounded-2xl border border-gray-100 bg-white p-5 shadow-lg shadow-gray-100/50 dark:border-gray-700/80 dark:bg-gray-800 dark:shadow-none">
                        <Link
                            href="/app/profile/edit"
                            className="absolute right-4 top-4 flex h-8 w-8 items-center justify-center rounded-full bg-green-50 text-[#15803d] dark:bg-green-950/50 dark:text-[#22c55e] transition hover:scale-105"
                            title="Edit Profil"
                        >
                            <Pencil size={14} />
                        </Link>
                        ...
                        <Field
                            label="Jenis Kelamin"
                            value={GENDER_LABEL[user.gender] ?? '-'}
                        />
                    ...
                    <nav className="mt-6">
                        ...
                        <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm dark:border-gray-700/80 dark:bg-gray-800">
                            <MenuLink
                                href="/app/profile/notifikasi"
                                icon={Bell}
                                label="Notifikasi"
                            />
                            <MenuLink
                                href="/app/profile/keamanan"
                                icon={ShieldCheck}
                                label="Keamanan Akun"
                                last
                            />
                    ...
                    <nav className="mt-6">
                        ...
                        <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm dark:border-gray-700/80 dark:bg-gray-800">
                            <MenuLink
                                href="/app/profile/bantuan"
                                icon={HelpCircle}
                                label="Pusat Bantuan"
                            />
                            <MenuLink
                                href="/app/profile/tentang"
                                icon={Info}
